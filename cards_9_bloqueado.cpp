...
vector<vector<int>> 
cartesian_product(vector<int> r1, vector<int> r2, vector<int> r3);
...
int main(){
    ...
    vector<vector<int>> product = cartesian_product(Ep, El, Ec);
    ...
    for(int i = 0; i < product.size(); i++) {
        ...
        write_csv(best_choices, best_results, "best_choices_9_bloqueado.csv");
...
tuple<vector<int>, vector<vector<int>>, vector<int>>
get_choice_cards(vector<int> three_cards, vector<int> deck){
    vector<int> choice(three_cards.begin(), three_cards.end());
    vector<int> X_s;
    ...
    for(int i = 0; i < n_cards; i++){
        ...
        for(int j = 0; j < choice.size() && !isRepetitive; j++)
            if(deck[i] == choice[j] || deck[i] == 107 || deck[i] == 110)
                isRepetitive = true;
        ...
        for(int i2 = i1+1; i2 < X_s.size(); i2++){
            for(int i3 = i2+1; i3 < X_s.size(); i3++){
                for(int i4 = i3+1; i4 < X_s.size(); i4++){
                    vector<int> line{X_s[i1], X_s[i2], X_s[i3], X_s[i4], 110, 107};
                    Ex.push_back(line);
                }
            }
        }
    ...
    // cout << "ok";
    return make_tuple(three_cards, Ex, X_s);
}
